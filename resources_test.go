...
var _ = Describe("Resources", func() {
	...
	Describe("Resource Matching", func() {
		...
		var err error
		BeforeEach(func() {
			requiredResource = rep.NewResource(10, 10, linuxRootFSURL, nil)
		})
		...
		Context("when no matching rootfs is found", func() {
			BeforeEach(func() {
				requiredResource.RootFs = "BAD_ROOTFS"
			})

			It("returns an error", func() {
				Expect(err).To(HaveOccurred())
				Expect(err).To(MatchError("rootfs not found"))
			})
		})
